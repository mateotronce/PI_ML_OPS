...
def userdata(User_id: str):
    ...
    if items == 0:
        return {"message": f"El usuario {User_id} no tiene ningún juego"}
    elif porcentaje != "No tiene reseñas":
        return {
            "message": f"El usuario {User_id} gastó {plata_gastada}$ en {items} juegos, "
                       f"también realizó {cantidad} reviews recomendando en un {porcentaje} de estas"
        }
    else:
        return {
            "message": f"El usuario {User_id} gastó {plata_gastada}$ en {items} juegos y no tiene reseñas"
        }
    
@app.get("/countrev/{fecha_inicio},{fecha_fin}")
def countreviews(fecha_inicio: str, fecha_fin: str):
    ...
    if total != 0:
        ...
        return {"message" : f"{usuarios} usuarios hicieron {cantidad} reviews, con un {porcentaje}% de reseñas positivas en un periodo de {diferencia_en_dias} días"}
    else:
        return {"message": "No hubo comentarios"}
...
def genre(genero: str):
    ...
    return {"message" : f"La columna '{genero}' está en la posición {posicion} con {horas} horas en total."}
...
def userforgenre(genero: str):
    ...
    # Crea una cadena para almacenar la información que deseas mostrar
    result = ""
    
    # Agrega el encabezado a la cadena
    result += f"Los 5 jugadores con más horas en {genero} son:\n"
    result += "puesto| usuario             | horas   | url_user\n"
    result += "-" * 50 + "\n"
    
    num = 1
    va = 0

    # Agrega los 5 mejores usuarios con formato a la cadena
    for i in range(len(top_5)):
        ...
        result += f"{num:<6}| {usuario:<20}| {horas:<8}| {url_user}\n"
        
        num += 1
        va += 1
    
    # Devuelve la cadena resultante como parte de la respuesta HTTP
    return result

@app.get("/developer/{publisher}")
def developer(publisher: str):
    ...
    # Agrega el encabezado a la cadena
    result += "Año     | Juegos | Contenido gratis\n"
    result += "-" * 30 + "\n"  # Línea de separación
    ...
    for año in empresa:
        ...
        if año[1] != "0":    
            ...
            result += f"{año[0]: <8} | {año[1]: <7} | {porcentaje}%\n"
    
    # Devuelve la cadena resultante como parte de la respuesta HTTP
    return result

@app.get("/sentiment/{fecha}")
def sentiment_analysis(fecha: int):
    r = []
    rev = re["Analisis"]
    ...
    # Realiza el conteo de sentimientos
    result += f"La cantidad es [Negativos:{contador_0}, Positivos:{contador_2}, Neutros:{contador_1}]"
    
    # Devuelve la cadena resultante como parte de la respuesta HTTP
    return result

@app.get("/recomendacionuser/{usuario}")
def recomendacionUseritem(usuario: str):
    ...
    result = "A usuarios como tú también les gustaron:\n"
    ...
    while num != 5: 
        try:
            ...
            result += f"Título: {juego['title'].iloc[0]}, Desarrollador: {juego['developer'].iloc[0]}\n"
            num += 1
        except:
            pass
...
@app.get("/recomendacionitem/{item}")
def recomendacionitemitem(nombre_juego: str):
    ...
    result = f"Si te gustan juegos como {nom_juego}, también te pueden gustar:\n"
    ...
    while num != 5:
        ...
        try:
            ...
            result += f"Juego: {game['app_name'].iloc[0]}, Desarrollador: {game['developer'].iloc[0]}\n"
            num += 1
            i += 1
        except:
            i += 1
